import s from './RecognizeHistoryElement.module.css'
...
export const RecognizeHistoryElement = (props) => {
    const {message} = App.useApp();
    const clipboard = useClipboard({timeout: 1000});
    return (
        <div className={s.container}>
            {
                props.emotion === 'positive' ?
                    <img className={s.emoji} src={PositiveEmoji} alt={'positive'}/> :
                    props.emotion === 'neutral' ?
                        <img className={s.emoji} src={NeutralEmoji} alt={'neutral'}/> :
                        <img className={s.emoji} src={NegativeEmoji} alt={'negative'}/>
            }
            <p className={s.text}>
                {props.text}
            </p>
