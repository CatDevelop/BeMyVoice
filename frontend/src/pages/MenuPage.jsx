...
export const MenuPage = (props) => {
    ...
    const intervalConversation = useInterval(() => {
        navigate('/conversation');
        intervalConversation.stop()
    }, 1300)


    const intervalServiceSubs = useInterval(() => {
        navigate('/servicesubs');
        intervalServiceSubs.stop()
    }, 1300)

    const intervalMicSub = useInterval(() => {
        navigate('/micsubs');
        intervalMicSub.stop()
    }, 1300)
            ...
            <div className={s.mainContainer}>
                ...
                <div className={s.menuPage}>
                    ...
                    <div className={s.menuCards}>
                        <div
                            className={classNames(s.card, isClosing ? s.closing : '')}
                            onClick={() => {
                                setIsClosing(true)
                                intervalConf.start()
                            }}
                        >
                            <img src={OnlineConferenceImage} alt={"Back"}/>

                            <Badge style={{position: 'absolute', left: '1rem', top: '1rem'}}
                                   size="sm"
                                   color="grape"
                                   variant="outline">Жесты</Badge>
                            <Badge style={{position: 'absolute', left: '4.8rem', top: '1rem'}}
                                   size="sm"
                                   color="indigo"
                                   variant="outline">Виртуальный микрофон</Badge>
                            <Badge style={{position: 'absolute', left: '1rem', top: '2.5rem'}}
                                   size="sm"
                                   color="violet"
                                   variant="outline">Распознавание с компьютера</Badge>

                            <div style={{position: 'absolute', left: '1rem', bottom: '1rem', width: '270px'}}>
                                <H4>
                                    <div style={{
                                        display: 'flex',
                        ...
                        <div className={classNames(s.card, isClosing ? s.closing : '')}
                             onClick={() => {
                                 setIsClosing(true)
                                 intervalConversation.start()

                             }}
                        >
                            <img src={TalkImage} alt={"Back"}/>
                            ...
                            <Badge style={{position: 'absolute', left: '4.8rem', top: '1rem'}}
                                   size="sm"
                                   color="indigo"
                                   variant="outline">Динамики</Badge>
                            <Badge style={{position: 'absolute', left: '1rem', top: '2.5rem'}}
                                   size="sm"
                                   color="violet"
                                   variant="outline">Распознавание с микрофона</Badge>
                        ...
                        <div className={classNames(s.card, isClosing ? s.closing : '')}
                             onClick={() => {
                                 setIsClosing(true)
                                 intervalServiceSubs.start()
                             }}
                        >
                            <img src={ServiceSubtitlesImage} alt={"Back"}/>
                            <Badge style={{position: 'absolute', left: '1rem', top: '1rem'}}
                                   size="sm"
                                   color="violet"
                                   variant="outline">Распознавание с компьютера</Badge>
                        ...
                        <div
                            ...
                            onClick={() => {
                                setIsClosing(true)
                                intervalMicSub.start()
                            }}
                        >
                            <img src={MicrophoneSubtitlesImage} alt={"Back"}/>
